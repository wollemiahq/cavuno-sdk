...
const OWN_BOARD = {
  [BOARD_ROUTE]: {
    body: '{"primaryDomain":"jobs.example.com","showCavunoBranding":false}',
  },
};
...
describe('read.attribution', () => {
  function brandingBoard(showCavunoBranding: boolean | undefined) {
    const body =
      showCavunoBranding === undefined
        ? '{"primaryDomain":"jobs.example.com"}'
        : JSON.stringify({
            primaryDomain: 'jobs.example.com',
            showCavunoBranding,
          });
    return { [BOARD_ROUTE]: { body } };
  }

  const withLink = `<html><body><div>home</div>
    <a href="https://cavuno.com">Powered by Cavuno</a>
    <a href="/companies/acme/jobs/eng">Eng</a></body></html>`;

  it.each([
    '<html>home</html>',
    '<html><div data-cavuno-attribution></div></html>',
    '<html><a hidden href="https://cavuno.com">Cavuno</a></html>',
  ])(
    'fails when branding is on without an actual visible link: %s',
    async (body) => {
      const results = await runReadProbes(
        healthyRoutes({ ...brandingBoard(true), '/': { body } }),
        'https://front.example',
        SEO,
        ENV,
      );
      const attribution = results.find((r) => r.id === 'read.attribution');
      expect(attribution?.status).toBe('fail');
      expect(attribution?.detail).toContain(
        'missing a crawlable Cavuno backlink',
      );
    },
  );

  it('passes with a backlink and shares the board fetch with the identity check', async () => {
    const fetchImpl = healthyRoutes({
      ...brandingBoard(true),
      '/': { body: withLink },
    });
    ...
    expect(results.find((r) => r.id === 'read.attribution')?.status).toBe(
      'pass',
    );
    expect(
      fetchImpl.mock.calls.filter(([url]) => String(url).endsWith(BOARD_ROUTE)),
    ).toHaveLength(1);
  });

  it('passes when branding is off even if the backlink is absent', async () => {
    const results = await runReadProbes(
      healthyRoutes(brandingBoard(false)),
      'https://front.example',
      SEO,
      ENV,
    );
    const attribution = results.find((r) => r.id === 'read.attribution');
    expect(attribution?.status).toBe('pass');
    expect(attribution?.detail).toContain('branding is off');
  });

  it('fails when the board endpoint cannot answer', async () => {
    const results = await runReadProbes(
      healthyRoutes({ [BOARD_ROUTE]: { status: 503, body: '{}' } }),
      'https://front.example',
      SEO,
  ...
  it('fails when the branding flag is absent from the board payload', async () => {
    const results = await runReadProbes(
      healthyRoutes(brandingBoard(undefined)),
      'https://front.example',
      SEO,
      ENV,
    );
  ...
  it('skips when env is absent', async () => {
    const results = await runReadProbes(
      healthyRoutes(),
      'https://front.example',
      SEO,
