...
describe('skill code examples reference real SDK symbols', () => {
  ...
  it('bites: a fabricated drift is detected', () => {
    ...
    expect(SUBPATH_EXPORTS['/format']!.has('formatSalaryStat')).toBe(true);
    expect(SUBPATH_EXPORTS['/format']!.has('formatSalaryStatRange')).toBe(true);
    expect(SUBPATH_EXPORTS['/format']!.has('formatSalaryStatUsd')).toBe(false);
    expect(SUBPATH_EXPORTS['/format']!.has('fullJobToCard')).toBe(false);
